...
|Already chosen section | Letters that have already been chosen appear below the hangman display so the user can see what they've already chosen. If the user chooses the same letter twice, an invalid input message will appear in yellow and they can re-enter. This does not count as a wrong answer. If the user guesses a word and it is wrong, it will appear in a list below the guessed letters list. The guessed words list will not appear until the user guesses a word incorrectly. | ![screenshot](readme-images/already-chosen-letters.png)  | ![screenshot](readme-images/wrong-word.png)  
|Win and loss screens| The end of game is marked with a splash of color on the win and loss screens with the invitation to either play again or quit the game. | ![screenshot](readme-images/win.png) | ![screenshot](readme-images/colors-4.png) 
...
| run.py | Python | Test the name input, name must have some input (i.e., user cannot just press enter) but input cannot be all numbers and cannot have characters except one space or a dash. | When invalid characters are entered, error messages display with instructions to try again. |![screenshot](readme-images/validation%20-name.png)
| run.py | Python | Test that only the number 1, 2 or 3 is entered to indicate difficulty level. | When invalid characters are entered, error messages display with instructions to try again. |![screenshot](readme-images/validation-difficulty-level.png)
| run.py | Python | Test that guess must be a letter or word | When invalid characters are entered, error messages display with instructions to try again. |![screenshot](readme-images/validation-not-a-letter.png)
| run.py | Python | Test that word guess is correct. | If the guessed word is incorrect, a message is displayed in yellow and the guessed word list appears.|![screenshot](readme-images/wrong-word.png)
| run.py | Python | Test that only Y or N is entered to play again or quit.| When invalid characters are entered, error messages display with instructions to try again. |![screenshot](readme-images/validation%20-%204.png)
